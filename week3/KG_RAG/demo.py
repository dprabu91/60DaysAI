...
DEMO_QUESTIONS = [
    "I have an on-premises database and I want a Live Connection on Tableau Cloud. What specific components and security credentials do I need?",
    "If I apply both a Context Filter and a FIXED LOD expression, which one determines the data seen by the other?",
    "Why would a developer prefer using Integers over Strings when building calculations for the Hyper engine?",
    "Can the Lifecycle Manager move a file to a Coldline bucket if the Compliance Engine has flagged it for an audit?",
    "Describe the lifecycle of data starting from Tableau Prep Builder and ending as a dashboard on Tableau Server.",
    "A user with 'Viewer' permissions wants to delete a bucket using an API Key. Is this possible? Why or why not?",
    "What is the fundamental difference between how a Live Connection and an Extract interact with the Hyper engine?"
]
...
async def initialize_systems():
    ...
    doc_path = Path("sample_data/Tableau.txt")
    ...
    if stats['total_nodes'] == 0:
        ...
        await kg_system.add_documents_to_graph(doc_texts, source="Tableau")
